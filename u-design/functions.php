...
if( is_admin() && function_exists('vc_set_as_theme') && (  function_exists( 'vc_license' ) && method_exists( vc_license(), 'isActivated') && !vc_license()->isActivated() ) ) {
    ...
    // VC update page message
    function vc_update_message( $slug, $item) {
            ...
            // Get TGMPA_List_Table class instance.
            $plugin_table = new TGMPA_List_Table;
            
            /*******************************( BEGIN Link to disable VC plugin )*********************************/
            foreach ( $tgmpa_instance->plugins as $plugin_slug => $plugin ) {
                if ( $slug === $plugin_slug ) {
                    // Only proceed forward if the plugin is active.
                    if ( ( isset($_GET['plugin_vc']) && $_GET['plugin_vc'] === 'disable' ) && $tgmpa_instance->is_plugin_active( $plugin_slug ) ) {
                        deactivate_plugins( $plugin['file_path'] );
                        ?>
                        <div class="notice notice-success is-dismissible">
                            <p><?php esc_html_e('Visual Composer has been disabled successfully. Please proceed with trying to update it again.', 'udesign'); ?></p>
                        </div>
                        <?php 
                    }
                }
            }
            // Generate new url for the tgmpa plugin update containing a query string to disable VC plugin
            $disable_vc_url = add_query_arg(
                array(
                    'plugin_status' => urlencode( 'update' ),
                    'plugin_vc' => urlencode( 'disable' ),
                ),
                $tgmpa_instance->get_tgmpa_url()
            );
            /*******************************( END Link to disable VC plugin )*********************************/
            
            // update page message
            echo '
                <tr class="plugin-update-tr">
                        <td colspan="', absint( $plugin_table->get_column_count() ), '" class="plugin-update colspanchange">
                                <div class="update-message">',
                                    '<strong>', sprintf(__("If you experience any issues updating the Visual Composer plugin from this page "
//                                            . "try deactivating it first from the %sPlugins page%s and then come back here to try updating it again.", "udesign"), 
                                            . "try deactivating it first by clicking %sthis link%s and then try updating it again.", "udesign"), 
//                                            '<a target="_blank" href="'.admin_url( 'plugins.php' ).'#wpbakery-visual-composer">',
                                            '<a href="'.esc_url( $disable_vc_url ).'">',
                                            '</a>'), '</strong>
                                </div>
                        </td>
                </tr>';
    }
    add_action( "tgmpa_after_plugin_row_js_composer", 'vc_update_message', 10, 2 );
}
...
<?php       // calculate top area height offset and custom styles
            ...
            if ( $top_area_over_header_image ) : ?>
                <script type="text/javascript">
                    // <![CDATA[
                    var headerOffsetHeight = document.getElementById('top-wrapper').offsetHeight;
...
<?php       endif; ?>
            <script type="text/javascript">jQuery('#custom-header-img').fadeIn( 1000 );</script>
